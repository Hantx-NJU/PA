...
/*#ifdef NEMU_REF_ALU
	...
	uint32_t res = 0;
        ...
        set_ZF(res, data_size);
        set_SF(res, data_size);
        cpu.eflags.OF = set_OF_adc(res, src, dest, data_size) || set_OF_adc(src + cpu.eflags.CF, src,cpu.eflags.CF , data_size) || set_OF_adc(dest + cpu.eflags.CF, cpu.eflags.CF, dest, data_size);
